    ...
    if((pid = vfork()) == 0) {
        ...
        if(cmd_flag->flag_pipe > 0) {
            ...
            while(1) {
                ...
                if(pid2 == 0) {
                    if(pipe_layer_var > 0) {
                        snprintf(tmp_file, CMD_FILENAME_LEN, "%s_%d",
                                 tmp_template, pipe_layer_var - 1);
                        fd1 = open(tmp_file, O_RDONLY, 0644);
                        dup2(fd1, 0);
                    }

                    if(pipe_layer_var < cmd_flag->flag_pipe) {
                        snprintf(tmp_file, CMD_FILENAME_LEN, "%s_%d",
                                 tmp_template, pipe_layer_var);
                        fd2 = open(tmp_file, O_WRONLY | O_CREAT |
                                   O_TRUNC, 0644);
                        dup2(fd2, 1);
                    }

                    //printf("pipe_layer = %d | pipe_cnt=%d <%s>\n",pipe_layer_var,pipe_cnt[pipe_layer_var],argv[pipe_cnt[pipe_layer_var]]);
                    execvp(argv[pipe_cnt[pipe_layer_var]],
                           &argv[pipe_cnt[pipe_layer_var]]);
                    exit(0);
                } else {
                    if(waitpid(pid2, NULL, 0) == -1) {
                        printf("wait child proc failed\n");
                    }
        ...
        case 0:
            ...
            for(; i < (unsigned int)this->cmdobj_num; ++i)
                ...
                == 0) {
                    ...
                    len = strlen((this->cmdobj + i)->CmdStr) - commond_len;
            ...
            if(j == 1) {
                ...
                while(j--) {
                    ...
                    if(len == 0) {
                        ...
                        if(new_len == 0) {
                            strncpy(new, patch[j] + commond_len, len + 1);
                            continue;
                        } else {
                            new_len = len > new_len ? new_len : len;

                            for(h = 0; h < new_len; ++h) {
                                if(*(char *)(patch[j] + commond_len + h) != *(new + h)) {
                                    break;
                                }
                            }
        ...
        if(ch == 27) {
            ...
            if(ch == 65) {	//key up and get cmd history
                ...
                if(i < this->history.cur_history_number) {
                    tmp = (tmp == this->history.entry) ?
                          (this-> history.entry + CMD_HISTORY_LEN - 1) : (tmp - 1);
                    ++i;	// up numbers
                ...
                if(i > 0) {
                    ...
                    printf("%s", buf);
                    fflush(stdout);
                    tmp = (tmp == (this->history.entry + CMD_HISTORY_LEN - 1))
                          ? this->history.entry : (tmp + 1);
                    --i;
                } else {
                    print_prompt(this);
                    printf("\a");
            ...
            strncpy(this->history.stack_pointer->cmd_buf, buf, buf_len);
            this->history.stack_pointer = (this->history.stack_pointer ==
                                           (this->history.entry + CMD_HISTORY_LEN - 1)) ?
                                          this->history.entry : (this->history.stack_pointer + 1);
            this->history.cur_history_number = (this->history.cur_history_number
                                                < CMD_HISTORY_LEN) ? (this->history.cur_history_number + 1)
                                               : CMD_HISTORY_LEN;
            printf("\n");
            /* reset_keypress(); */
            patch_flag = 0;
